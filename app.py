...
app = FastAPI(title="PDF & Document Tools API", version="1.1.0")
...
async def extract_text(file: UploadFile = File(...), authorized: bool = Depends(verify_api_key)):
    t0 = time.time()
...
@app.post("/summary")
async def summary(file: UploadFile = File(...), sentences: int = Query(5, ge=1, le=20), authorized: bool = Depends(verify_api_key)):
    t0 = time.time()
    size = _ensure_size_ok(file)
    text = extract_text_from_upload(file)
...
@app.post("/word-count")
async def word_count(file: UploadFile = File(...), authorized: bool = Depends(verify_api_key)):
    t0 = time.time()
    size = _ensure_size_ok(file)
    text = extract_text_from_upload(file)
